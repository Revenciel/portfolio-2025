...
import audit from '@/public/images/multi-brand-design-system/audit.webp'
import colorThemes from '@/public/images/multi-brand-design-system/color-themes.webp'
import components from '@/public/images/multi-brand-design-system/components.webp'
import designTokens from '@/public/images/multi-brand-design-system/design-tokens.webp'
import siteComparison from '@/public/images/multi-brand-design-system/site-comparison.webp'
import stakeholderNeeds from '@/public/images/multi-brand-design-system/stakeholder-needs.webp'
    ...
    <div className='wrapper'>
        ...
        <div className='summary-card'>
            ...
            <div>
                <h3>Outcomes</h3>
                    <ul>
                        <li>Reduced time-to-launch on new sites from 6 to 3 weeks</li>
                        <li>Eliminated accessibility & consistency issues</li>
                        <li>Accelerated A/B testing capabilities</li>
                    </ul>
            </div>
        </div>
...
<H2group eyebrow="Challenge + Opportunity" heading="Tidying Up with a Design System
    ...
    At Home Solutions, a small team of devs and one designer (that’s me!) managed marketing landing pages for over a dozen brands. We had to work quickly to keep up with partner requests while continuously testing changes to improve conversion rates. But working so fast came at a cost: inconsistencies forced devs to recreate similar designs from scratch every time we made a new site. Content editors frequently violated accessibility guidelines because there weren’t enough guardrails for them in the <abbr title='Content Management System'>CMS</abbr>. And our sites didn’t look trustworthy, because our piecemeal approach to improvements resulted in a lack of internal consistency. In short, we had a mess on our hands.

    When our engineering director announced we were going to migrate to a new CMS, I proposed standardizing all our sites with a multi-brand design system. Since we were going to re-code our websites anyway, the team was receptive when I explained how a design system would drastically accelerate the launching of new sites, streamline implementing <abbr title='Conversion Rate Optimization'>CRO</abbr> successes across brands, and bake accessibility into the process.
</Section>
...
<Section theme=''>
    ...
    ### Auditing the Current Site Portfolio
